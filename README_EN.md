...
<p align="center">
  <sub>11 AI agents form the Three Departments & Six Ministries: Planning triages & proposes, Review vetoes, Dispatch assigns, Ministries execute.<br>Built-in <b>institutional review gates</b> that CrewAI doesn't have. A <b>real-time dashboard</b> that AutoGen doesn't have.</sub>
</p>
...
```
You (Emperor) → Planning Dept (Triage + Plan) → Review Dept → Dispatch Dept → 6 Ministries → Report Back
   皇上              中书省               门下省         尚书省           六部          回奏
```

This isn't a cute metaphor. It's **real separation of powers** for AI:

- **Planning (中书省)** triages messages — casual chat gets auto-replied, real commands become tasks
...
|---|:---:|:---:|:---:|:---:|
| **Built-in triage + review** | ❌ | ⚠️ | ⚠️ | **✅ Dedicated triage + reviewer** |
...
### 🤖 Let Claw Deploy for You (Recommended for OpenClaw Users)

If you're using **OpenClaw**, simply tell your claw:

> "https://github.com/838997125/Tang-Political-System.git Download this project to `<your-directory>` and install it"

**Example:**
```
https://github.com/838997125/Tang-Political-System.git Download this project to D:\tools and install it
```

Your claw will handle:
- ✅ Clone the project to the specified directory
- ✅ Detect and install dependencies (OpenClaw, Python)
- ✅ Run the installer and handle execution policies
- ✅ Configure 10 agents (Three Departments & Six Ministries)
- ✅ Start the system and verify it's running

**After completion, your claw will tell you:**
- Project installation location
- How to start the system
- How to access the dashboard (http://127.0.0.1:7891)
...
|------|----------|------|-----------|
| 📜 **Planning** | `zhongshu` | Triage, receive, plan, decompose | Chat detection, requirements, architecture |
...
> **Permission Notes:**
> - **Planning**: Central dispatcher, can submit to Review or consult Dispatch
> - **Review**: Returns to Planning only (approve/reject), cannot directly call Dispatch
> - **Dispatch**: Coordinates ministries, can escalate to Review, reports back to Planning
> - **Ministries**: Report to Dispatch only, no cross-department direct communication

### State Machine

```
Emperor → Planning (Triage+Plan) → Review → Assigned → Executing → ✅ Done
                                         ↑                       │
                                         └── Veto ──┘   Blocked ──
```
...
```
edict/
├── agents/                     # 11 agent personality templates (SOUL.md)
│   ├── zhongshu/               #   Planning Dept (triage + plan)
│   ├── menxia/                 #   Review Dept
...
What you’ll find:
- 🏛️ Architecture deep-dives — how 10 agents achieve separation of powers
