...
DASHBOARD_URL = os.environ.get('EDICT_DASHBOARD_URL', 'http://127.0.0.1:7891')
API_TOKEN = None

# Try to find .env from multiple locations
def find_env_file():
    """Find .env file from various possible locations"""
    # Priority 1: Environment variable
    env_path = os.environ.get('EDICT_ENV_FILE')
    if env_path and pathlib.Path(env_path).exists():
        return pathlib.Path(env_path)
    
    # Priority 2: Same directory as script (for edict-main)
    env_file = _BASE / '.env'
    if env_file.exists():
        return env_file
    
    # Priority 3: Look for edict-main in parent directories
    current = _BASE
    for _ in range(5):  # Search up 5 levels
        ...
        if (parent / 'dashboard' / 'server.py').exists():
            # Found edict-main root
            return parent / '.env'
        current = parent
    
    # Priority 4: Default edict-main location
    default = pathlib.Path('D:/code/edict-main/.env')
    if default.exists():
        return default
...
def push_to_dashboard(task):
    """Push task update to dashboard via HTTP API"""
    token = load_api_token()
    if not token:
        log.warning('API token not found, skipping dashboard push')
        # Try to find env file for debugging
        env_file = find_env_file()
        if env_file:
            log.info(f'Found .env at: {env_file}')
        else:
            log.warning('No .env file found in any location')
        return False
    ...
    try:
        ...
        url = f"{DASHBOARD_URL}/api/tasks/update"
        data = json.dumps({'task': task}).encode('utf-8')
        
        log.info(f"Pushing task {task['id']} to {url}")
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if result.get('ok'):
                log.info(f"Pushed task {task['id']} to dashboard")
                return True
            else:
                log.warning(f"Dashboard push failed: {result.get('error')}")
                return False
...
def save_and_push(task):
    """Save task and push to dashboard"""
    tasks = load()
    # Remove existing task with same id
    tasks = [t for t in tasks if t.get('id') != task['id']]
    tasks.insert(0, task)
    save(tasks)
    # Push to dashboard via HTTP API
    push_to_dashboard(task)
