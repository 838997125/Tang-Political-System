...
> 🚨 **必须完成以下所有步骤！不完成视为任务失败！**
> 
> 执行后验证：确认 `data/morning_brief.json` 文件已更新，且包含今日数据。
...
3. 同时触发刷新：
   ```bash
   py scripts/refresh_live_data.py  # 在项目根目录下执行
   ```
...
```bash
py scripts/kanban_update.py progress JJC-xxx "正在采集全球新闻，已完成政治/军事类" "政治新闻采集✅|军事新闻采集✅|经济新闻采集🔄|AI新闻采集|生成简报"
```
