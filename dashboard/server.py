...
"""
import json, pathlib, subprocess, sys, threading, argparse, datetime, logging, re, os, platform
...
try:
    ...
    if IS_WINDOWS:
        ...
        log.warning('psutil 未安装，进程检测功能可能受限。建议安装: pip install psutil')
...
DASHBOARD = BASE                # dashboard.html 所在目录
...
def load_api_token():
    ...
    if env_file.exists():
        ...
        for line in content.split('\n'):
            if line.startswith('EDICT_API_TOKEN='):
                return line.split('=', 1)[1].strip()
    return None

API_TOKEN = load_api_token()
if API_TOKEN:
    log.info(f'API Token loaded (length: {len(API_TOKEN)})')
else:
    log.warning('No API Token found in .env, event-driven sync will be disabled')
...
def save_tasks(tasks):
    ...
    def _refresh():
        try:
            ...
            subprocess.run([python_cmd, str(SCRIPTS / 'refresh_live_data.py')], timeout=30)
...
def add_skill_to_agent(agent_id, skill_name, description, trigger=''):
    ...
    try:
        python_cmd = _get_python_cmd()
        subprocess.run([python_cmd, str(SCRIPTS / 'sync_agent_config.py')], timeout=10)
    except Exception:
        pass
...
def _check_gateway_alive():
    """检测 Gateway 进程是否在运行。支持 Windows 和 Unix。"""
    # 首先尝试 HTTP probe，这是最可靠的检测方式
    if _check_gateway_probe():
        return True
    
    # 如果 HTTP probe 失败，再尝试进程检测
    try:
        if HAS_PSUTIL:
            # 使用 psutil 跨平台检测
            for proc in psutil.process_iter(['name', 'cmdline']):
                try:
                    ...
                    if 'openclaw' in name.lower() or 'openclaw' in cmdline_str.lower():
                        return True
...
def _check_agent_process(agent_id):
    """检测是否有该 Agent 的 openclaw-agent 进程正在运行。支持 Windows 和 Unix。"""
    try:
        if HAS_PSUTIL:
            # 使用 psutil 跨平台检测
            for proc in psutil.process_iter(['name', 'cmdline']):
                try:
                    ...
                    cmdline = proc.info.get('cmdline') or []
                    cmdline_str = ' '.join(cmdline) if cmdline else ''
                    if agent_id in cmdline_str and 'openclaw' in name.lower():
                        return True
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    continue
            return False
            ...
            return agent_id in result.stdout and 'openclaw' in result.stdout.lower()
        else:
            # Unix/Linux/macOS 使用 pgrep
            result = subprocess.run(
...
def handle_scheduler_retry(task_id, reason=''):
    ...
    sched['lastDispatchTrigger'] = 'zhongshu-retry'
    _scheduler_add_flow(task, f'触发重试第{sched["retryCount"]}次：{reason or "超时未推进"}')
    task['updatedAt'] = now_iso()
    save_tasks(tasks)

    dispatch_for_state(task_id, task, state, trigger='system-retry')
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 中书省调度自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
    ...
    if snap_state not in _TERMINAL_STATES:
        dispatch_for_state(task_id, task, snap_state, trigger='zhongshu-rollback')
...
def handle_scheduler_scan(threshold_sec=180):
    ...
    for task in tasks:
        ...
        if retry_count < max_retry:
            ...
            sched['lastRetryAt'] = now_iso()
            sched['lastDispatchTrigger'] = 'zhongshu-scan-retry'
    ...
    for task_id, state in pending_retries:
        ...
        if retry_task:
            dispatch_for_state(task_id, retry_task, state, trigger='zhongshu-scan-retry')
    ...
    for task_id, state in pending_rollbacks:
        ...
        if rollback_task and state not in _TERMINAL_STATES:
            dispatch_for_state(task_id, rollback_task, state, trigger='zhongshu-auto-rollback')
...
# 状态推进顺序（手动推进用）
_STATE_FLOW = {
    'Pending':  ('Zhongshu', '皇上', '中书省', '待处理旨意转交中书省起草'),
...
_STATE_LABELS = {
    'Pending': '待处理', 'Zhongshu': '中书省', 'Menxia': '门下省',
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ('', '/dashboard', '/dashboard.html'):
            self.send_file(DASHBOARD / 'dashboard.html')
        elif p == '/healthz':
    ...
    def do_POST(self):
        ...
        if p == '/api/morning-brief/refresh':
            ...
            def do_refresh():
                try:
                    python_cmd = _get_python_cmd()
                    cmd = [python_cmd, str(SCRIPTS / 'fetch_morning_news.py')]
                    if force:
                        cmd.append('--force')
        ...
        if p == '/api/tasks/update':
            # Event-driven task sync endpoint
            # Verify Authorization header
            auth_header = self.headers.get('Authorization', '')
            if not API_TOKEN:
                self.send_json({'ok': False, 'error': 'API token not configured on server'}, 500)
                return
            ...
            if not task or not isinstance(task, dict):
                self.send_json({'ok': False, 'error': 'task object required'}, 400)
                return
            if 'id' not in task:
                self.send_json({'ok': False, 'error': 'task.id required'}, 400)
                return
            
            # Update tasks_source.json
            try:
                ...
                atomic_json_update(DATA / 'tasks_source.json', update_tasks, [])
                
                # Trigger refresh_live_data.py
                def refresh_async():
                    try:
                        python_cmd = _get_python_cmd()
                        subprocess.run([python_cmd, str(SCRIPTS / 'refresh_live_data.py')], 
                ...
                self.send_json({'ok': True, 'message': f"Task {task['id']} updated"})
            except Exception as e:
                self.send_json({'ok': False, 'error': f'Update failed: {e}'}, 500)
            return
        ...
        if p == '/api/set-model':
            ...
            def apply_async():
                try:
                    python_cmd = _get_python_cmd()
                    subprocess.run([python_cmd, str(SCRIPTS / 'apply_model_changes.py')], timeout=30)
                    subprocess.run([python_cmd, str(SCRIPTS / 'sync_agent_config.py')], timeout=10)
                except Exception as e:
...
def main():
    ...
    # 启动时同步 agent 配置和运行时数据
    log.info('正在同步数据...')
    try:
        import subprocess
        # 同步 agent 配置
        result1 = subprocess.run(
            [sys.executable, str(SCRIPTS / 'sync_agent_config.py')],
            capture_output=True,
            text=True,
            timeout=30
        )
        if result1.returncode == 0:
            log.info('Agent 配置同步成功')
        else:
            log.warning(f'Agent 配置同步失败: {result1.stderr}')
        
        # 同步运行时数据
        result2 = subprocess.run(
            [sys.executable, str(SCRIPTS / 'sync_from_openclaw_runtime.py')],
            capture_output=True,
            text=True,
            timeout=30
        )
        if result2.returncode == 0:
            ...
            log.warning(f'运行时数据同步失败: {result2.stderr}')
    except Exception as e:
        log.warning(f'数据同步出错: {e}')
